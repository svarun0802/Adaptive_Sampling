...
def run_comprehensive_foundation():
    ...
    # Same 20 training queries as before...
    training_queries = [
        ("Simple Count", "SELECT COUNT(*) FROM customer"),
        ("Simple Filter", "SELECT COUNT(*) FROM supplier WHERE s_region = 'AMERICA'"),
        ("Basic Aggregation", "SELECT c_region, COUNT(*) FROM customer GROUP BY c_region"),
        ("Customer Revenue", "SELECT c_region, SUM(lo_revenue), COUNT(*) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey GROUP BY c_region"),
        ("Supplier Analysis", "SELECT s_nation, SUM(lo_quantity), AVG(lo_supplycost) FROM lineorder lo JOIN supplier s ON lo.lo_suppkey = s.s_suppkey GROUP BY s_nation"),
        ("Part Analysis", "SELECT p_category, SUM(lo_extendedprice), COUNT(*) FROM lineorder lo JOIN part p ON lo.lo_partkey = p.p_partkey GROUP BY p_category"),
        ("Date Analysis", "SELECT d_year, SUM(lo_revenue), COUNT(*) FROM lineorder lo JOIN date_dim d ON lo.lo_orderdate = d.d_datekey GROUP BY d_year"),
        ("Customer Detailed", "SELECT c_region, c_nation, SUM(lo_revenue), AVG(lo_quantity) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey GROUP BY c_region, c_nation"),
        ("Customer Revenue 2", "SELECT c_region, AVG(lo_revenue), MAX(lo_revenue) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey GROUP BY c_region"),
        ("Supplier Analysis 2", "SELECT s_nation, COUNT(*), SUM(lo_quantity) FROM lineorder lo JOIN supplier s ON lo.lo_suppkey = s.s_suppkey GROUP BY s_nation"),
        ("Part Analysis 2", "SELECT p_category, AVG(lo_extendedprice), COUNT(DISTINCT lo_orderkey) FROM lineorder lo JOIN part p ON lo.lo_partkey = p.p_partkey GROUP BY p_category"),
        ("Customer-Supplier", "SELECT c_region, s_nation, SUM(lo_revenue), COUNT(*) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey JOIN supplier s ON lo.lo_suppkey = s.s_suppkey GROUP BY c_region, s_nation"),
        ("Customer-Part", "SELECT c_region, p_category, SUM(lo_revenue), AVG(lo_extendedprice) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey JOIN part p ON lo.lo_partkey = p.p_partkey GROUP BY c_region, p_category"),
        ("Supplier-Part", "SELECT s_nation, p_mfgr, SUM(lo_quantity), COUNT(*) FROM lineorder lo JOIN supplier s ON lo.lo_suppkey = s.s_suppkey JOIN part p ON lo.lo_partkey = p.p_partkey GROUP BY s_nation, p_mfgr"),
        ("Customer-Date", "SELECT c_region, d_year, SUM(lo_revenue), COUNT(*) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey JOIN date_dim d ON lo.lo_orderdate = d.d_datekey GROUP BY c_region, d_year"),
        ("Supplier-Date", "SELECT s_nation, d_year, SUM(lo_quantity), AVG(lo_supplycost) FROM lineorder lo JOIN supplier s ON lo.lo_suppkey = s.s_suppkey JOIN date_dim d ON lo.lo_orderdate = d.d_datekey GROUP BY s_nation, d_year"),
        ("4-Table Complex 1", "SELECT c_region, s_nation, p_category, SUM(lo_revenue) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey JOIN supplier s ON lo.lo_suppkey = s.s_suppkey JOIN part p ON lo.lo_partkey = p.p_partkey GROUP BY c_region, s_nation, p_category"),
        ("4-Table Complex 2", "SELECT c_region, s_region, d_year, SUM(lo_revenue), COUNT(*) FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey JOIN supplier s ON lo.lo_suppkey = s.s_suppkey JOIN date_dim d ON lo.lo_orderdate = d.d_datekey GROUP BY c_region, s_region, d_year"),
        ("4-Table Complex 3", "SELECT d_year, c_nation, p_mfgr, SUM(lo_revenue - lo_supplycost) as profit FROM lineorder lo JOIN customer c ON lo.lo_custkey = c.c_custkey JOIN part p ON lo.lo_partkey = p.p_partkey JOIN date_dim d ON lo.lo_orderdate = d.d_datekey GROUP BY d_year, c_nation, p_mfgr"),
    ...
    for i, (name, query) in enumerate(training_queries, 1):
        ...
        if result and result.get('success'):
            ...
            if sample_info and sample_info.get('newly_created') and sample_info.get('materialized'):
                ...
                print(f"Q{i:2d}: {name:18} | {exec_time:6.1f}ms | {strategy:12} | ✅ MATERIALIZED ({sample_id})")
            elif sample_info and sample_info.get('reused_existing'):
                reuses += 1
                sample_id = sample_info.get('sample_id', 'unknown')[:8]
                print(f"Q{i:2d}: {name:18} | {exec_time:6.1f}ms | {strategy:12} | ♻️  REUSED ({sample_id})")
            elif strategy.startswith('sampling'):
                print(f"Q{i:2d}: {name:18} | {exec_time:6.1f}ms | {strategy:12} | 📝 In-memory")
            else:
                print(f"Q{i:2d}: {name:18} | {exec_time:6.1f}ms | {strategy:12} | ⚡ Direct")
    
    actual_samples = system.sample_storage.list_materialized_samples()
    print(f"\nFOUNDATION COMPLETE: {len(actual_samples)} materialized samples | {reuses} reuses")
...
def show_confidence_intervals(confidence_intervals):
    """Display confidence intervals if available"""
    if not confidence_intervals or not any(confidence_intervals):
        return
    
    print("     📊 95% Confidence Intervals:")
    for i, row_ci in enumerate(confidence_intervals[:2]):
        if row_ci and any(ci for ci in row_ci if ci):
            ci_str = " | ".join(
                f"[{ci[0]:,.0f}-{ci[1]:,.0f}]" if ci else "N/A" 
                for ci in row_ci[:3]
            )
            print(f"       Row {i+1}: {ci_str}")

def run_strategic_performance_comparison(system):
    """Phase 2: Strategic queries designed to show sampling benefits"""
    ...
    for i, test in enumerate(performance_tests, 1):
        ...
        if intelligent_result and intelligent_result.get('success'):
            ...
            if intelligent_result.get('confidence_interval'):
                show_confidence_intervals(intelligent_result['confidence_interval'])
    ...
    if successful_tests > 0:
        ...
        print(f"⏱️  Total exact time: {total_exact_time:.1f}ms")
        ...
        if sampling_used_count > 0:
            ...
            print(f"   ⚠️  System chose exact execution for all queries")
